...
// Constantes
const COUT_ANIMATEUR_5J = 450;  // 5 jours
const COUT_ANIMATEUR_4J = 360;  // 4 jours
const COUT_AIDE_5J = 225;      // 5 jours
const COUT_AIDE_4J = 180;      // 4 jours
const SEMAINES_4_JOURS = ['Pâques S1 2025'];  // Semaines avec jours fériés
...
window.changerStatutPaiement = function(index) {
    ...
    button.className = `btn btn-sm ${getBadgeClass(nouveauStatut)}`;
    
    // Mettre à jour les résumés
    const activeFilter = document.querySelector('#presenceStagesTabs .nav-link.active');
    const stageFiltre = activeFilter ? 
        activeFilter.id.replace('presence-', '').replace('-tab', '') : 
        'tous';
...
window.telechargerFicheInformation = function(index) {
    ...
    const data = [
        ...
        ['Niveau VTT', inscription.stage.includes('VTT') ? 
            (inscription.niveauVTT ? 'Perfectionnement' : 'Débutant') : 
            'Non applicable'],
...
function calculerTotalParStage() {
    ...
    inscriptions.forEach(inscription => {
        ...
        if (!totaux[key]) {
            ...
            totaux[key].coutsSalariaux = animateursStage.reduce((sum, a) => {
                if (SEMAINES_4_JOURS.includes(a.semaine)) {
                    return sum + (a.role === 'Animateur' ? COUT_ANIMATEUR_4J : COUT_AIDE_4J);
                } else {
                    return sum + (a.role === 'Animateur' ? COUT_ANIMATEUR_5J : COUT_AIDE_5J);
                }
            }, 0);
        }
        
        const montant = parseFloat(inscription.montant) || 0;
    ...
    fraisFonctionnement.forEach(frais => {
        ...
        totaux[key].fraisFonctionnement += parseFloat(frais.montant) || 0;
    });
...
function mettreAJourTableauDeBord() {
    ...
    inscriptions.forEach(inscription => {
        ...
        // Vérifier le statut de paiement et ajouter au total correspondant
        switch(inscription.paiement) {
            case 'Payé':
                totalPercu += montant;
                break;
            case 'Non payé':
                totalARecevoir += montant;
                break;
            case 'Liquide':
                totalLiquide += montant;
                break;
            // Pas besoin de gérer 'Offert' car il n'est pas compté dans les totaux
        }
    });

    // Afficher les totaux avec 2 décimales
...
document.addEventListener('DOMContentLoaded', function() {
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-2">
                                <div>
                                    ${donnees.count > 0 ? `
                                        ...
                                        ${donnees.participants.map(participant => {
                                            const inscr = inscriptions.find(i => i.nom === participant);
                                            if (inscr.stage === 'VTT') {
                                                return `${participant} ${inscr.niveauVTT ? '(Perfectionnement)' : '(Débutant)'}`;
                                            }
                                            return participant;
                                        }).join(', ')}
                                    ` : 'Aucun inscrit'}
    ...
    function afficherInscriptions(stageFiltre = null) {
        ...
        inscriptionsTriees.forEach((inscription, index) => {
            ...
            row.innerHTML = `
                ...
                <td>
                    ${inscription.stage === 'VTT' ? 
                        `VTT ${inscription.niveauVTT ? '(Perfectionnement)' : '(Débutant)'}` : 
                        inscription.stage}
                </td>
    ...
    form.addEventListener('submit', function(e) {
        ...
        const niveauVTT = stage === 'VTT' ? document.getElementById('niveauVTT').value === 'true' : null;
        const CAPACITE_MAX = 20;
    ...
    function creerGraphiques(totaux, stageFiltre = null) {
        ...
        // Regrouper les données VTT
        const totauxCombines = {};
        for (const [key, value] of Object.entries(totaux)) {
            const [stage, semaine] = key.split('-');
            const newKey = `${stage === 'VTT' ? 'VTT' : stage}-${semaine}`;
            
            if (!totauxCombines[newKey]) {
                totauxCombines[newKey] = {
                    paye: 0,
                    nonPaye: 0,
                    liquide: 0,
                    coutsSalariaux: 0,
            ...
            totauxCombines[newKey].paye += value.paye;
            totauxCombines[newKey].nonPaye += value.nonPaye;
            totauxCombines[newKey].liquide += value.liquide;
            totauxCombines[newKey].coutsSalariaux += value.coutsSalariaux;
            totauxCombines[newKey].fraisFonctionnement += value.fraisFonctionnement;
        }

        for (const [key, value] of Object.entries(totauxCombines)) {
            const [stage, semaine] = key.split('-');
            
            // Filtrer selon le type de stage
            if (stageFiltre) {
                if (stageFiltre === 'vtt' && stage !== 'VTT') continue;
                if (stageFiltre === 'artistique-culinaire' && stage !== 'Artistique/Culinaire') continue;
                if (stageFiltre !== 'vtt' && stageFiltre !== 'artistique-culinaire' && 
                    !stage.toLowerCase().startsWith(stageFiltre.toLowerCase())) continue;
            }
    ...
    document.getElementById('profilAnimateurForm').addEventListener('submit', function(e) {
        ...
        const nouveauProfil = {
            ...
            registreNational: document.getElementById('registreNationalAnimateur').value,
...
window.modifierInscription = function(index) {
    ...
    // Gérer le niveau VTT
    const niveauVTTDiv = document.getElementById('niveauVTTDiv');
    if (inscription.stage.includes('VTT')) {
        niveauVTTDiv.style.display = 'block';
        document.getElementById('niveauVTT').value = inscription.niveauVTT ? 'true' : 'false';
    } else {
        niveauVTTDiv.style.display = 'none';
    }
...
window.exporterListePresence = function(stage, semaine) {
    ...
    if (inscritsStage.length > 0) {
        const listePresence = inscritsStage.map(i => ({
            'Nom/Prénom': i.stage === 'VTT' ? 
                `${i.nom} ${i.niveauVTT ? '(Perfectionnement)' : '(Débutant)'}` : 
                i.nom,
            'Âge': i.age,
            'Parents': i.parents,
...
window.exporterCoutsSalariaux = function() {
    ...
    animateurs.forEach(a => {
        const cout = SEMAINES_4_JOURS.includes(a.semaine) ?
            (a.role === 'Animateur' ? COUT_ANIMATEUR_4J : COUT_AIDE_4J) :
            (a.role === 'Animateur' ? COUT_ANIMATEUR_5J : COUT_AIDE_5J);
