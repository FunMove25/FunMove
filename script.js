                ...
                <div class="payments-pending">
                    ${paiementsEnAttente.map(inscription => `
                        <div class="payment-item">
                            ...
                            Parent: ${inscription.parents1.nom}<br>
                            Tél: ${inscription.parents1.telephone}
                        </div>
                    `).join('')}
                </div>
            </div>
    ...
    function afficherInscriptions(stageFiltre = null) {
        ...
        inscriptionsTriees.forEach((inscription, index) => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <button class="btn btn-sm btn-info me-2" onclick="telechargerFicheInformation(${index})">
                        <i class="bi bi-file-earmark-text"></i> Fiche info
                    </button>
                    <button class="btn btn-sm btn-danger" onclick="desinscrireEnfant(${index})">
...
window.telechargerFicheInformation = function(index) {
    ...
    // Créer les données pour la fiche
    const data = [
        ['Fiche d\'information'],
        [''],
    ...
    // Mettre en forme la feuille
    ws['!cols'] = [{ wch: 20 }, { wch: 40 }]; // Largeur des colonnes
    ...
    for (let i = 0; i < data.length; i++) {
        if (data[i][0] && !data[i][1]) { // Pour les titres de section
            ws[XLSX.utils.encode_cell({ r: i, c: 0 })].s = {
